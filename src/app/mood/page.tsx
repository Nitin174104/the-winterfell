...
import { Music, ArrowRight, Loader2, ArrowLeft } from "lucide-react";
      ...
      <div className="max-w-5xl w-full relative z-10">
        ...
        <div className="grid md:grid-cols-2 gap-8 items-stretch">
            ...
            <motion.div 
                ...
                className="glass-card p-10 rounded-[2.5rem] h-full flex flex-col justify-between border-purple-500/20 hover:border-purple-500/50 transition-all hover:bg-purple-500/5"
            >
                <div>
                    <h2 className="text-3xl font-bold mb-4">Auto-Detect Vibe</h2>
                    <p className="text-gray-400 mb-8 text-lg leading-relaxed">We'll analyze your recent listening history to find the perfect flavor match.</p>
                </div>
                <button 
                    onClick={handleSpotifyConnect}
                    disabled={isConnecting}
                    className="w-full py-5 bg-[#1DB954] hover:bg-[#1ed760] text-black font-bold rounded-2xl flex items-center justify-center gap-3 transition-transform active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed text-lg shadow-[0_0_30px_rgba(29,185,84,0.3)] hover:shadow-[0_0_50px_rgba(29,185,84,0.5)]"
                >
                    {isConnecting ? <Loader2 className="animate-spin" /> : <Music size={24} />}
                    {isConnecting ? "Analyzing Vibe..." : "Connect Spotify"}
                </button>
            </motion.div>
